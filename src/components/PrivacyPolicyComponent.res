@module external css: {..} = "./PrivacyPolicyComponent.module.scss"

@react.component
let make = (~className="") => {
  let {component: module(Modal), openModal, isOpen} = Modal.use()

  <>
    <button
      className={Cx.cx([css["privacyPolicyButton"], className])} type_="button" onClick={openModal}>
      {"Политика конфиденциальности"->React.string}
    </button>
    {isOpen
      ? <Modal>
...
3.  Актуальная версия Политики в свободном доступе расположена в сети Интернет по адресу https://pantheon.fun.`}
            />
          </div>
        </Modal>
      : React.null}
